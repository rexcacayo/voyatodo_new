...
var status='';
var baseurl = window.location.protocol +"//" + window.location.host + '/projects/voyatodo/web/';
	...
	$.fn.eCalendar = function( options ) {
		...
		var el				= this,
			defaults 		= {
				...
				ajaxDayLoader		: baseurl+ 'advertiser/getdays?id='+idproduct,													// php file that returns event days of current month
		...
		this.init = function() {
			...
			if( el.find('.hb-months').length == 0 && el.find('.hb-days').length == 0 ) {
				el.html(
					'<div class="hb-months">' +
						'<a href="javascript:;" class="hb-change-month hb-prev-month" data-month="" data-year=""><i class="fa fa-chevron-left"></i></a>' +
						'<a href="javascript:;" class="hb-current-month" data-month="" data-year=""></span>' +
						'<a href="javascript:;" class="hb-change-month hb-next-month" data-month="" data-year=""><i class="fa fa-chevron-right"></i></a>' +
					'</div>' +
		...
		this.initMonths = function() {
			...
			var act_prev = '<div class="hb-months"><a href="javascript:;" class="hb-change-month hb-prev-month " data-month="" data-year=""><i class="fa fa-chevron-left"></i></a>';
			var act_next = '<a href="javascript:;" class="hb-change-month hb-next-month " data-month="" data-year=""><i class="fa fa-chevron-right"></i></a></div>';
		...
		el.on('click', '.hb-day', function() {
			...
			if($(this).hasClass("hb-day-active")){
				$(this).attr("class","hb-day");
				$.ajax({
					type: 'post',
					data:{
						date:date,
					...
					url: baseurl + 'advertiser/deletedate',
					success: function(data){
						if(data != ''){
							var json = "";
							$.each(eval(data.replace(/[\r\n]/, "")), function(i,item){
								$(".json_dates").val(json += item.pkavailability+',');
							});
						}
					}
				});
				...
				$.ajax({
					...
					url: baseurl + 'advertiser/createdate',
					success: function(data){
						if(data != ''){
							var json = $(".json_dates").val();
	        				$(".json_dates").val(json.concat(data+','));
						}
					}
				});
			}
